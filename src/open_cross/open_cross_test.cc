            ...
            for (int y=0; y<height; ++y) {
                for (int x=0; x<width; ++x) {
                    ...
                    for (int j = -(window_height/2); j < -(window_height/2) + window_height; j++) {
                        ...
                        miny = f(miny, workbuf[(itr%2)*width*height*depth + (x*height + yy)*depth + c]);
                    }
                    for (int i = -(window_width/2); i < -(window_width/2) + window_width; i++) {
                        int xx = x + i >= 0 ? x + i: 0;
                        xx = xx < width ? xx : width - 1;
                        minx = f(minx, workbuf[(itr%2)*width*height*depth + (xx*height + y)*depth + c]);
                    }
                    workbuf[((itr+1)%2)*width*height*depth + (x*height + y)*depth + c] = f(minx, miny);
                }
            }
        }
    }
