                ...
                for (int x=0; x<res_width; ++x) {
                    ...
                    for (int tmp_y=0; tmp_y<tmp_height; ++tmp_y) {
                        for (int tmp_x=0; tmp_x<tmp_width; ++tmp_x) {
                            double diff = static_cast<double>(input0(x+tmp_x, y+tmp_y, c)) - static_cast<double>(input1(tmp_x, tmp_y));
                            expect += abs(diff);
                        }
                    }
...
#endif //~USE_CUDA
#ifdef TYPE_u8
    if (test<uint8_t>(tm_sad_u8)) return 1;
#endif
#ifdef TYPE_u16
    if (test<uint16_t>(tm_sad_u16)) return 1;
#endif
#ifdef TYPE_u32
    if (test<uint32_t>(tm_sad_u32)) return 1;
#endif
    return 0;
}
