                     ...
                     struct halide_buffer_t *_dst_buffer))
{
    try {
        ...
        expect = warp_map_bilinear_ref(expect, input0, input1, input2, border_value, border_type, width, height, depth);
        for (int c=0; c<depth; ++c) {
            // for each x and y
            for (int j=0; j<width; ++j) {
                for (int i=0; i<height; ++i) {
...
#endif //~USE_CUDA
#ifdef TYPE_u8
    if (test<uint8_t>(warp_map_bilinear_u8)) return 1;
#endif
#ifdef TYPE_u16
    if (test<uint16_t>(warp_map_bilinear_u16)) return 1;
#endif
    return 0;
}
