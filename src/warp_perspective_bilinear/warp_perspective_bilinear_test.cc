...

{
    try {
        ...
        transform(6) = 0.0; transform(7) = 0.0; transform(8) = 1.0;
...
#endif //~USE_CUDA
#ifdef TYPE_u8
    if (test<uint8_t>(warp_perspective_bilinear_u8)) return 1;
#endif
#ifdef TYPE_u16
    if (test<uint16_t>(warp_perspective_bilinear_u16)) return 1;
#endif
    return 0;
}
