...
//
template<typename T>
GeneratorInput<Buffer<T>>&
schedule(GeneratorInput<Buffer<T>>& gi,
         ...
         const std::vector<Expr>& extents)
{
    ...
    for (size_t i = 0; i < mins.size(); ++i) {
        gi.dim(i).set_bounds(mins[i], extents[i])
          .dim(i).set_stride(i ? gi.dim(i - 1).stride() * extents[i - 1] : 1);
    }
         ...
         const std::vector<Expr>& extents)
{
    ...
    for (size_t i = 0; i < mins.size(); ++i) {
        ip.dim(i).set_bounds(mins[i], extents[i])
          .dim(i).set_stride(i ? ip.dim(i - 1).stride() * extents[i - 1] : 1);
    }
...
template<typename T>
GeneratorOutput<Buffer<T>>&
schedule(GeneratorOutput<Buffer<T>>& go,
         const std::vector<Expr>& mins,
         const std::vector<Expr>& extents)
{
    assert(mins.size() == extents.size());
...
template<typename T>
GeneratorInput<Buffer<T>>&
schedule(GeneratorInput<Buffer<T>>& gi, const std::vector<Expr>& shape)
{
...
ImageParam&
schedule(ImageParam& ip, const std::vector<Expr>& shape)
{
    const std::vector<Expr> mins(shape.size(), 0);
...
template<typename T>
GeneratorOutput<Buffer<T>>&
schedule(GeneratorOutput<Buffer<T>>& go, const std::vector<Expr>& shape)
{
    const std::vector<Expr> mins(shape.size(), 0);
...
Func&
schedule(Func& f, const std::vector<Expr>& shape)
{
    const std::vector<Expr> mins(shape.size(), 0);
...
Func&
schedule_burst(Func& f, const std::vector<Expr>& shape, Expr burst_num)
{
    schedule(f, shape);
...
ImageParam&
schedule_burst(ImageParam& ip, Expr burst_num)
{
#if defined(HALIDE_FOR_FPGA)
    Expr bn = simplify(burst_num);
    throw_assert(is_const(bn), "burst_num should be constant value.");
...
Buffer<>&
schedule_burst(Buffer<>& b, Expr burst_num)
{
#if defined(HALIDE_FOR_FPGA)
    Expr bn = simplify(burst_num);
    throw_assert(is_const(bn), "burst_num should be constant value.");
...
Func&
schedule_memory(Func& f, const std::vector<Expr>& shape)
{
    schedule(f, shape);
#if defined(HALIDE_FOR_FPGA)
...
ImageParam&
set_extent_interval(ImageParam& ip,
                    const std::vector<std::pair<int, int>>& intervals)
{
    ...
    for (size_t i = 0; i < intervals.size(); ++i) {
        ip.dim(i).set_min(0);
#if defined(HALIDE_FOR_FPGA)
        ip.dim(i).set_extent_interval(Interval(intervals[i].first, intervals[i].second));
#endif
    }
...
Func&
set_extent_interval(Func& f,
                    const std::vector<std::pair<int, int>>& intervals)
{
    using Halide::Internal::Interval;

    assert(static_cast<size_t>(f.dimensions()) >= intervals.size() &&
           f.output_buffer().defined());

    for (size_t i = 0; i < intervals.size(); ++i) {
        f.output_buffer().dim(i).set_min(0);
#if defined(HALIDE_FOR_FPGA)
        f.output_buffer().dim(i).set_extent_interval(Interval(intervals[i].first, intervals[i].second));
#endif
    }
