...
template<typename T>
Func warp_map_NN(GeneratorInput<Buffer<T>> &src0,
                 ...
                 int32_t width, int32_t height, int32_t depth)
{
    ...
    Func type0 =
        BoundaryConditions::constant_exterior(src0, border_value,
                                              {{0, width}, {0, height}, {0, depth}});
    Func type1 =
        BoundaryConditions::repeat_edge(src0,
                                        {{0, width}, {0, height}, {0, depth}});
...
template<typename T>
Func warp_map_bilinear(GeneratorInput<Buffer<T>> &src0,
                       GeneratorInput<Buffer<float>> &src1,
                       GeneratorInput<Buffer<float>> &src2,
                       int32_t border_type, Expr border_value,
                       int32_t width, int32_t height, int32_t depth)
{
    ...
    Func type0 =
        BoundaryConditions::constant_exterior(src0, border_value,
                                              {{0, width}, {0, height}, {0, depth}});
    Func type1 =
        BoundaryConditions::repeat_edge(src0, {{0, width}, {0, height}, {0, depth}});
                      ...
                      int32_t width, int32_t height, int32_t depth)
{
    ...
    Expr w0 = ((a * (dx + 1.0f) - 5.0f * a) *
              (dx + 1.0f) + 8.0f * a) * (dx + 1.0f) - 4.0f * a;
...
template<typename T>
Func warp_affine_NN(GeneratorInput<Buffer<T>> &src,
                    int32_t border_type, Expr border_value,
                    ...
                    int32_t width, int32_t height, int32_t depth)
{
    ...
    Expr i = unsafe_promise_clamped(cast<int>(floor(srcy)),
                                    -height, 2 * height);
    Expr j = unsafe_promise_clamped(cast<int>(floor(srcx)),
                                    -width, 2 * width);
...
template<typename T>
Func warp_affine_bilinear(GeneratorInput<Buffer<T>> &src,
                          int32_t border_type, Expr border_value,
                          GeneratorInput<Buffer<double>> &transform,
                          int32_t width, int32_t height, int32_t depth)
{
...
template<typename T>
Func warp_affine_bicubic(GeneratorInput<Buffer<T>> &src,
                         int32_t border_type, Expr border_value,
                         GeneratorInput<Buffer<double>> &transform,
                         int32_t width, int32_t height, int32_t depth)
{
    ...
    Expr w0 = ((a * (dx + 1.0f) - 5.0f * a) * (dx + 1.0f) + 8.0f * a) *
              (dx + 1.0f) - 4.0f * a;
    Expr w1 = ((a + 2.0f) * dx - (a + 3.0f)) * dx * dx + 1.0f;
    Expr w2 = ((a + 2.0f) * (1.0f - dx) - (a + 3.0f)) * (1.0f - dx) *
              (1.0f - dx) + 1.0f;
    Expr w3 = 1.0f - w2 - w1 - w0;
    ...
    w0 = ((a * (dy + 1.0f) - 5.0f * a) * (dy + 1.0f) + 8.0f * a) *
         (dy + 1.0f) - 4.0f * a;
    w1 = ((a + 2.0f) * dy - (a + 3.0f)) * dy * dy + 1.0f;
    w2 = ((a + 2.0f) * (1.0f - dy) - (a + 3.0f)) * (1.0f - dy) *
         (1.0f - dy) + 1.0f;
    w3 = 1.0f - w2 - w1 -w0;
