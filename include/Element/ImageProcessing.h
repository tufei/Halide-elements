                  ...
                  int32_t depth, const bool auto_schedule = false)
{
    ...
    for (int pass_size = 1; pass_size < size; pass_size <<= 1) {
        for (int chunk_size = pass_size; chunk_size > 0; chunk_size >>= 1) {
            next = Func("bitonic_pass_" + std::to_string(pass_size) + "_" +
            ...
            if (pass_size == chunk_size && pass_size > 1) {
                ...
                next(i, x, y, c) = select(i < chunk_middle,
                                          min(prev(i, x, y, c),
                                              prev(partner, x, y, c)),
                                          max(prev(i, x, y, c),
                                              prev(partner, x, y, c)));
                ...
                // Regular pass
                Expr partner = chunk_start + (chunk_index + chunk_size) %
                               (chunk_size * 2);
                next(i, x, y, c) = select(i < chunk_middle,
                                          min(prev(i, x, y, c),
                                              prev(partner, x, y, c)),
...
template<typename T>
Func median(GeneratorInput<Buffer<T>> &src, int32_t width, int32_t height,
            int32_t depth, int32_t window_width, int32_t window_height,
            const bool auto_schedule = false)
{
    Func in = src;
    ...
    Var x{"x"}, y{"y"}, i{"i"}, c{"c"};
    window(i, x, y, c) = undef(in.value().type());
    window((r.x + offset_x) + (r.y + offset_y) * window_width, x, y, c) =
    ...
    Func sorted = bitonic_sort(window, window_size, width, height, depth,
                               auto_schedule);
    Func median("median");
    median(x, y, c) = sorted(window_size / 2, x, y, c);

    if (!auto_schedule) {
        sorted.compute_at(median, x);
        schedule(window, {window_size, width, height, depth});
    }
...
template<typename T>
Func laplacian(GeneratorInput<Buffer<T>> &in, int32_t width, int32_t height,
               int32_t depth, const bool auto_schedule = false)
{
    Var x{"x"}, y{"y"}, c{"c"};
    ...
    RDom r(-1, 3, -1, 3);
    Func dst("dst");
    Expr dstval = sum(cast<double>(clamped(x + r.x, y + r.y, c)) *
                                   kernel(r.x, r.y));
    dstval = select(dstval < 0, -dstval, dstval);
    dstval = select(dstval > type_of<T>().max(),
...
template <typename T>
Func tm_ssd(GeneratorInput<Buffer<T>> &src0, GeneratorInput<Buffer<T>> &src1,
...
template <typename T>
Func tm_ncc(GeneratorInput<Buffer<T>> &src0, GeneratorInput<Buffer<T>> &src1,
            const int32_t img_width, const int32_t img_height,
            const int32_t tmp_width, const int32_t tmp_height)
{
    Var x{"x"}, y{"y"}, c{"c"};
    ...
    sum1 = sum(cast<double>(src0(x + r.x, y + r.y, c)) *
               cast<double>(src1(r.x, r.y)));
    sum2 = sum(cast<double>(src0(x + r.x, y + r.y, c)) *
               cast<double>(src0(x + r.x, y + r.y, c)));
             ...
             const int32_t tmp_width, const int32_t tmp_height)
{
    ...
    avr0 = sum(cast<double>(src0(x + r.x, y + r.y, c))) / tmp_size;
...
template <typename T>
Func split3(GeneratorInput<Buffer<T>> &src, int32_t width, int32_t height)
{
    Var x{"x"}, y{"y"};
    Func dst{"dst"};
...
template <typename T>
Func split4(GeneratorInput<Buffer<T>> &src, int32_t width, int32_t height)
{
    Var x{"x"}, y{"y"};
    Func dst{"dst"};

    dst(x, y) = Tuple(src(x, y, 0), src(x, y, 1), src(x, y, 2), src(x, y, 3));

    return dst;
}

template<typename T>
Func sad(GeneratorInput<Buffer<T>> &input0, GeneratorInput<Buffer<T>> &input1,
         int32_t width, int32_t height, int32_t depth)
{
    Var x{"x"}, y{"y"}, c{"c"};
...
template<typename T>
Func warp_map_bilinear(GeneratorInput<Buffer<T>> &src0, GeneratorInput<Buffer<float>> &src1, GeneratorInput<Buffer<float>> &src2, int32_t border_type, Expr border_value, int32_t width, int32_t height, int32_t depth)
{
    Var x{"x"}, y{"y"}, c{"c"};
    Func dst{"dst"};
...
template<typename T>
Func warp_map_bicubic(GeneratorInput<Buffer<T>> &src0,
                      GeneratorInput<Buffer<float>> &src1,
                      GeneratorInput<Buffer<float>> &src2,
                      ...
                      int32_t width, int32_t height, int32_t depth)
{
    ...
    Func type0 = BoundaryConditions::constant_exterior(src0, border_value,
                                                       0, width,
                                                       0, height,
                                                       0, depth);
    Func type1 = BoundaryConditions::repeat_edge(src0,
                                                 {{0, width},
                                                  {0, height},
                                                  {0, depth}});
    ...
    Expr dx = min(max(0.0f, j-xf-1.0f), 1.0f);
    Expr dy = min(max(0.0f, i-yf-1.0f), 1.0f);
...
template<typename T>
Func warp_affine_NN(GeneratorInput<Buffer<T>> &src, int32_t border_type, Expr border_value, GeneratorInput<Buffer<double>> &transform, int32_t width, int32_t height, int32_t depth)
{
    Var x{"x"};
    Var y{"y"};
    Var c{"c"};
...
template<typename T>
Func warp_affine_bilinear(GeneratorInput<Buffer<T>> &src, int32_t border_type, Expr border_value, GeneratorInput<Buffer<double>> &transform, int32_t width, int32_t height, int32_t depth)
{
    Var x{"x"};
    Var y{"y"};
    Var c{"c"};
...
template<typename T>
Func warp_affine_bicubic(GeneratorInput<Buffer<T>> &src, int32_t border_type, Expr border_value, GeneratorInput<Buffer<double>> &transform, int32_t width, int32_t height, int32_t depth)
{
    Var x{"x"};
    Var y{"y"};
    Var c{"c"};
...
template <typename T>
Func threshold_tozero(GeneratorInput<Buffer<T>> &src, Expr threshold)
{
    Var x{"x"}, y{"y"}, c{"c"};
    Func dst;
    dst(x, y, c) = select (src(x, y, c) > threshold, src(x, y, c), 0);

    return dst;
}

template <typename T>
Func threshold_tozero_inv(GeneratorInput<Buffer<T>> &src, Expr threshold)
{
    Var x{"x"}, y{"y"}, c{"c"};
    Func dst;
    dst(x, y, c) = select (src(x, y, c) > threshold, 0, src(x, y, c));
...
template<typename T>
Func label_firstpass(GeneratorInput<Buffer<T>> &src,
                     ...
                     const bool auto_schedule = false)
{
    ...
    Var x{"org_x"}, y{"org_y"};
    original(x, y) = cast<int32_t>(select(src(x,y) == 0,
                                   MAX, x + y * width + 1));

    Func extend = BoundaryConditions::constant_exterior(original, MAX,
                                                        {{0, width},
                                                         {0, height}});
    Func copy{"copy"};
                               ...
                               min(findMin(s.x, s.y),
                                   min(min(findMin(s.x - 1, s.y - 1),
                                       ...
                                       min(findMin(s.x + 1, s.y - 1),
                                           findMin(s.x-1, s.y)))));
    if (!auto_schedule) findMin.compute_root();

    Func backward{"back"}, whereUp{"where"};
    whereUp(x, y) = select(findMin(x, y) == MAX,
                           MAX,
    ...
    if (!auto_schedule) whereUp.compute_root();

    backward(x, y) = findMin(x, y);
    Expr indX{"indx"},indY{"indy"};
    indX = (findMin(s.x, s.y) - 1) % width;
    indY = select((findMin(s.x, s.y) - 1) / width < height,
                  (findMin(s.x, s.y) - 1) / width, height - 1);
    Expr target = backward(clamp(indX, 0, width), clamp(indY, 0, height));

    Expr tarX = (whereUp(s.x, s.y) - 1) % width;
    Expr tarY = select((whereUp(s.x, s.y) - 1) / width < height,
                       (whereUp(s.x, s.y) - 1) / width, height - 1);
    Expr origin = backward(clamp(tarX, 0, width), clamp(tarY, 0, height));

    backward(clamp(indX, 0, width), clamp(indY, 0, height)) =
        select(target > whereUp(s.x, s.y),
               min(whereUp(s.x, s.y), origin),
               target);
    if (!auto_schedule) backward.compute_root();
                             ...
                             min(final(s.x, s.y),
                                 min(min(final(s.x - 1, s.y - 1),
                                     ...
                                     min(final(s.x + 1, s.y - 1),
                                         final(s.x - 1, s.y)))));
    if (!auto_schedule) final.compute_root();
...
Func label_secondpass(GeneratorInput<Buffer<uint32_t>> &src,
                      GeneratorInput<Buffer<uint32_t>> &buf,
                      int32_t width, int32_t height, Expr bufW,
                      const bool auto_schedule = false)
{
    Func secondPass;
    Var x, y;
