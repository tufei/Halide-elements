...
template<typename T>
Func min_pos(GeneratorInput<Buffer<T>> &src, int32_t width, int32_t height,
...
template<typename T>
Func max_value(GeneratorInput<Buffer<T>> &src,
               GeneratorInput<Buffer<uint8_t>> &roi,
               int32_t width, int32_t height, int32_t depth,
               const bool auto_schedule = false)
{
...
template<typename T, typename D>
Func sq_integral(GeneratorInput<Buffer<T>> &src, int32_t width, int32_t height,
                 int32_t depth)
{
    Var x{"x"}, y{"y"}, c{"c"};
    Func dst{"dst"};
